...
export default function ProductDetail() {
  ...
  const agregarAlCarrito = () => {
    setCarrito(prev => {
      // Guardamos la configuración elegida en este instante para este producto individual
      const nuevoItem = {
        ...producto,
        idUnicoInterno: Date.now() + Math.random(), // Evita colisiones si meten el mismo producto con planes distintos
        price: precioBase,
        cantidad: 1,
      ...
      <div style={{ maxWidth: '1100px', margin: '40px auto', padding: '0 20px', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: window.innerWidth < 768 ? '1fr' : '1fr 1fr', gap: '40px', backgroundColor: colors.bgCards, borderRadius: '20px', padding: window.innerWidth < 768 ? '20px' : '40px', border: colors.borderCard, boxShadow: colors.shadow }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff', borderRadius: '14px', padding: '20px', border: '1px solid #e2e8f0', minHeight: '300px', maxHeight: '450px' }}>
            <img src={producto.image_url} alt={producto.title || 'Producto'} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              >
                ...
                {producto.stock > 0 ? 'Consultar esta opción' : 'Sin existencias'}
              </button>
              ...
              >
                🛒 {producto.stock > 0 ? 'Añadir al Carrito de Pedidos' : 'Agotado'}
              </button>
            </div>
          </div>
        </div>
