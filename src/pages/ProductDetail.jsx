...
export default function ProductDetail() {
  ...
  const calcularCuotaIndividual = () => {
    ...
    if (modalidad === 'financiado') return Math.round((precioBase / (1 - (tasa / 100))) / cuotasElegidas);
    if (modalidad === 'tarjeta') return Math.round(precioBase / cuotasElegidas);
    return 0; 
  };

  const calcularPrecioTotalCliente = () => {
    if (!producto) return 0;
    if (modalidad === 'financiado') return Math.round(producto.price / (1 - (tasasExcel[cuotasElegidas] || 0) / 100));
  ...
  const agregarAlCarrito = () => {
    ...
    setCarrito(prev => {
      ...
      if (existe) {
        ...
        return [...prev, { id: producto.id, title: producto.title, price: producto.price, image_url: producto.image_url, cantidad: 1 }];
      }
    });
    setMostrarCarrito(true); // Abre el panel para mostrarle que se agregó
  };
  ...
  const enviarCarritoCompletoPorWhatsApp = () => {
    ...
    let mensaje = "Hola SOL & LUNA, quiero realizar el siguiente pedido múltiple:\n\n";
    let total = 0;
    ...
    mensaje += `\n*Total a pagar: Gs. ${total.toLocaleString('es-PY')}*\n\n¿Tienen disponibilidad de todos estos artículos y cómo sería el envío/retiro?`;
    ...
    <div style={{ minHeight: '100vh', backgroundColor: colors.bgPrincipal, color: colors.textoBlanco, fontFamily: 'system-ui, sans-serif' }}>
      ...
      {carrito.length > 0 && (
        <button 
          ...
          style={{ position: 'fixed', bottom: '25px', right: '25px', backgroundColor: colors.colorAcento, color: '#fff', border: 'none', borderRadius: '50%', width: '60px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', cursor: 'pointer', boxShadow: '0 10px 25px rgba(0,0,0,0.3)', zIndex: 999, transition: 'transform 0.2s' }}
        >
          🛒
          <span style={{ position: 'absolute', top: '0px', right: '0px', backgroundColor: colors.colorRojo, color: '#fff', fontSize: '12px', fontWeight: 'bold', width: '22px', height: '22px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: `2px solid ${colors.bgPrincipal}` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: window.innerWidth < 768 ? '1fr' : '1fr 1fr', gap: '40px', backgroundColor: colors.bgCards, borderRadius: '20px', padding: window.innerWidth < 768 ? '20px' : '40px', border: colors.borderCard, boxShadow: colors.shadow }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <div>
              ...
              <div style={{ backgroundColor: colors.bgInputs, border: `1px solid ${colors.borderInputs}`, borderRadius: '12px', padding: '20px', marginBottom: '25px' }}>
                <div style={{ display: 'flex', gap: '4px', marginBottom: '15px', backgroundColor: colors.bgPrincipal, padding: '4px', borderRadius: '8px' }}>
                  <button type="button" onClick={() => setModalidad('financiado')} style={{ flex: 1, padding: '8px 4px', fontSize: '11px', fontWeight: 'bold', border: 'none', borderRadius: '6px', cursor: 'pointer', backgroundColor: modalidad === 'financiado' ? colors.colorAcento : 'transparent', color: modalidad === 'financiado' ? '#fff' : colors.textoGris }}>Pagar en Cuotas</button>
                  <button type="button" onClick={() => setModalidad('tarjeta')} style={{ flex: 1, padding: '8px 4px', fontSize: '11px', fontWeight: 'bold', border: 'none', borderRadius: '6px', cursor: 'pointer', backgroundColor: modalidad === 'tarjeta' ? colors.colorAcento : 'transparent', color: modalidad === 'tarjeta' ? '#fff' : colors.textoGris }}>Tarjeta de Crédito</button>
                  <button type="button" onClick={() => setModalidad('contado')} style={{ flex: 1, padding: '8px 4px', fontSize: '11px', fontWeight: 'bold', border: 'none', borderRadius: '6px', cursor: 'pointer', backgroundColor: modalidad === 'contado' ? colors.colorAcento : 'transparent', color: modalidad === 'contado' ? '#fff' : colors.textoGris }}>Al Contado</button>
                </div>
                  ...
                  <>
                    <div style={{ marginBottom: '15px' }}>
                      ...
                      <select value={cuotasElegidas} onChange={(e) => setCuotasElegidas(parseInt(e.target.value))} style={{ width: '100%', padding: '10px', borderRadius: '6px', backgroundColor: colors.bgPrincipal, border: `1px solid ${colors.borderInputs}`, color: colors.textoBlanco, fontSize: '14px', outline: 'none', cursor: 'pointer' }}>
                        {Object.keys(tasasExcel).map(mes => <option key={mes} value={mes}>{mes} meses</option>)}
                      </select>
                    </div>
                    ...
                    <div style={{ textAlign: 'center', backgroundColor: colors.bgPrincipal, padding: '15px', borderRadius: '8px', border: `1px solid ${colors.borderInputs}` }}>
                      ...
                      <span style={{ fontSize: '26px', fontWeight: '800', color: '#10b981', display: 'block', margin: '4px 0' }}>Gs. {calcularCuotaIndividual().toLocaleString('es-PY')} <small style={{ fontSize: '12px', color: colors.textoGris, fontWeight: 'normal' }}>/ mes</small></span>
                      {modalidad === 'financiado' && <span style={{ fontSize: '11px', color: colors.textoGris }}>Valor total del artículo financiado: Gs. {calcularPrecioTotalCliente().toLocaleString('es-PY')}</span>}
                    </div>
                  </>
                ) : (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <button 
                onClick={comprarUnicoPorWhatsApp} disabled={producto.stock <= 0}
                style={{ width: '100%', padding: '16px', backgroundColor: producto.stock > 0 ? '#25D366' : colors.borderInputs, color: '#fff', border: 'none', borderRadius: '12px', cursor: producto.stock > 0 ? 'pointer' : 'not-allowed', fontWeight: 'bold', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', boxShadow: producto.stock > 0 ? '0 4px 15px rgba(37, 211, 102, 0.3)' : 'none' }}
              >
                ...
                {producto.stock > 0 ? 'Consultar disponibilidad' : 'Sin existencias'}
              </button>

              <button 
                onClick={agregarAlCarrito} disabled={producto.stock <= 0}
                style={{ width: '100%', padding: '16px', backgroundColor: 'transparent', color: producto.stock > 0 ? colors.colorAcento : colors.textoGris, border: `2px solid ${producto.stock > 0 ? colors.colorAcento : colors.borderInputs}`, borderRadius: '12px', cursor: producto.stock > 0 ? 'pointer' : 'not-allowed', fontWeight: 'bold', fontSize: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', transition: 'background-color 0.2s' }}
                onMouseEnter={e => { if(producto.stock>0) {e.target.style.backgroundColor = colors.colorAcento; e.target.style.color = '#fff'} }}
                onMouseLeave={e => { if(producto.stock>0) {e.target.style.backgroundColor = 'transparent'; e.target.style.color = colors.colorAcento} }}
              >
                🛒 {producto.stock > 0 ? 'Agregar al Carrito de Pedidos' : 'Agotado'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div style={{ width: window.innerWidth < 768 ? '100%' : '450px', backgroundColor: colors.bgCards, height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '-10px 0 25px rgba(0,0,0,0.5)', animation: 'slideIn 0.3s forwards' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
              {carrito.length === 0 ? (
                <div style={{ textAlign: 'center', color: colors.textoGris, marginTop: '50px' }}>
                  ...
                  <p style={{ fontSize: '13px' }}>Agrega productos para armar un pedido múltiple.</p>
                </div>
              ) : (
            ...
            {carrito.length > 0 && (
              <div style={{ padding: '25px 20px', borderTop: `1px solid ${colors.borderInputs}`, backgroundColor: colors.bgInputs }}>
                ...
                <button 
                  onClick={enviarCarritoCompletoPorWhatsApp}
                  style={{ width: '100%', padding: '16px', backgroundColor: '#25D366', color: '#fff', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold', fontSize: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', boxShadow: '0 4px 15px rgba(37, 211, 102, 0.3)' }}
                >
                ...
                <p style={{ textAlign: 'center', fontSize: '11px', color: colors.textoGris, marginTop: '15px', marginBottom: 0 }}>
                  * El envío de este mensaje no representa un compromiso de compra.
                </p>
              </div>
            )}
          </div>
      ...
      {/* Agregar un poco de CSS para la animación del carrito */}
      <style>{`
        @keyframes slideIn {
